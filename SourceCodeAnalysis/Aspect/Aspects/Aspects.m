...
/// 对比 blockSignature 和原方法的方法签名
/// @param blockSignature block 的方法签名
/// @param object 原方法对应的对象
/// @param selector 原方法
/// @param error error
...
static void aspect_prepareClassAndHookSelector(NSObject *self, SEL selector, NSError **error) {
    NSCParameterAssert(selector);
    // 若 selector 是实例方法，则 klass 是self 动态生成一个子类 xxx_Aspects_, 并将 self 的 isa 指针指向子类
    ...
    if (!aspect_isMsgForwardIMP(targetMethodIMP)) {
        ...
        // 给子类添加 selector 方法，并将方法实现转发给 _objc_msgForward
...
static Class aspect_hookClass(NSObject *self, NSError **error) {
    ...
    // **** 能来到这里 说明 hook 的必然是实例方法 *****
	const char *subclassName = [className stringByAppendingString:AspectsSubclassSuffix].UTF8String;
	Class subclass = objc_getClass(subclassName);
//    id sub;
//    struct zz_objc_class *subClass;
//    struct zz_objc_class *subMetaClass;
...
//        sub = [subclass new];
//        subClass = (__bridge struct zz_objc_class *)(object_getClass(sub));
//        subMetaClass = (__bridge struct zz_objc_class *)(object_getClass(subclass));
//
...
static void aspect_swizzleForwardInvocation(Class klass) {
    ...
    // class_replaceMethod 解释如下:
    // 1>若类(元类)中有 forwardInvocation:,则将 forwardInvocation: 的 IMP 替换为 __ASPECTS_ARE_BEING_CALLED__, 并返回方法原来的 IMP
    // 2>若类(元类)中没有 forwardInvocation:方法, 则为子类新增 forwardInvocation: 方法, 对应的IMP就是传入的imp, 返回 nil
    
//    zz_getAllMethods(klass);
//    NSLog(@"\n\n");
...
//    NSLog(@"\n\n");
    ...
    if (originalImplementation) {
        // 若类(元类)中有 forwardInvocation:，则动态添加 __aspects_forwardInvocation: 方法，__aspects_forwardInvocation: 方法实现为 originalImplementation
...
static void __ASPECTS_ARE_BEING_CALLED__(__unsafe_unretained NSObject *self, SEL selector, NSInvocation *invocation) {
    ...
    // originalSelector 添加 apsects__ 前缀
	SEL aliasSelector = aspect_aliasForSelector(invocation.selector);
    // 替换 selector (所以bolck中参数 aspectInfo.originalInvocation.selector 返回的是 aliasSelector)
    invocation.selector = aliasSelector;
    
    // 关联对象获取 objectContainer
    AspectsContainer *objectContainer = objc_getAssociatedObject(self, aliasSelector);
    // 关联对象获取 classContainer
...
static AspectsContainer *aspect_getContainerForObject(NSObject *self, SEL selector) {
    NSCParameterAssert(self);
    // 构造 aspects_xxx 的 aliasSelector
    SEL aliasSelector = aspect_aliasForSelector(selector);
    // 加了前缀的 aliasSelector 当关联对象中的 key, 来获取 AspectsContainer
...
static BOOL aspect_isSelectorAllowedAndTrack(NSObject *self, SEL selector, AspectOptions options, NSError **error) {
    ...
    if (class_isMetaClass(object_getClass(self))) {
        ...
        // dispatch_once 初始化一个空字典 [class: AspectTracker]
...
// 就是对 aspect_isSelectorAllowedAndTrack 中是元类判断的反操作
...
+ (instancetype)identifierWithSelector:(SEL)selector object:(id)object options:(AspectOptions)options block:(id)block error:(NSError **)error {
    ...
    // 对比 block 的方法签名和 selector 的方法签名
    if (!aspect_isCompatibleBlockSignature(blockSignature, object, selector, error)) {
        return nil;
    }
    
    // 方法签名验证通过，将 selector、block 等存储起来
